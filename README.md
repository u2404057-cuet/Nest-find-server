...
```
Nest-find-server/
├── index.ts          # Express app, MongoDB connection, and all API routes
...
> The server is intentionally kept as a single `index.ts` file — all routes, the MongoDB connection (`MongoClient`), and helper functions like `parseId()` for safely converting string IDs to Mongo `ObjectId`s live here. TypeScript compiles to `dist/` via `tsc`.
